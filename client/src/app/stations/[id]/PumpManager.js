...
export default function PumpManager({ stationId, initialPumps }) {
  ...
  return (
    <div className="mt-6">
      ...
      {/* Formik form for adding pumps - only show when showAddForm is true */}
      {showAddForm && (
        <Formik
        ...
        >
          {() => (
            <Form className="mt-4 bg-card p-4 rounded-lg border border-gray-200">
          ...
          {pumps.map((pump) => (
            ...
            >
              {editingPumpId === pump.id ? (
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="font-medium text-gray-900">{pump.pump_number}</h4>
                    <p className="text-sm text-gray-600">{pump.fuel_type}</p>
                  </div>
